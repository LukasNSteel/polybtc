...
def main() -> None:
    ...
    # collateral sanity (warn, don't block — a dry run needs no funds)
    try:
        ba = client.get_balance_allowance(
            BalanceAllowanceParams(asset_type=AssetType.COLLATERAL, signature_type=0))
        log.info("collateral balance/allowance: %s", ba)
    ...
    ask_px = round(ask_px, 3)
    shares = args.usd / ask_px
    if shares < MIN_SHARES:  # respect the venue's 5-share floor so it isn't auto-rejected
        shares = MIN_SHARES
        log.warning("$%.2f is below the 5-share minimum at %.3f; bumping to %.0f sh (~$%.2f)",
                    args.usd, ask_px, shares, shares * ask_px)
    shares = round(shares, 2)
    amount = round(shares * ask_px, 2)
    log.info("plan: BUY %s %s  %.2f sh @ %.3f  = $%.2f collateral (book ask size %.0f)",
             mk["title"], side.upper(), shares, ask_px, amount, ask_sz)
    ...
    def build():
        return client.create_market_order(
            MarketOrderArgs(token_id=token, amount=amount, side=Side.BUY,
                            price=ask_px, order_type=OrderType.FAK), opts)
    ...
    if isinstance(resp, dict):
        ...
        try:
            if making and taking and float(taking) > 0:
                implied = (float(making) / float(taking) - avg) * filled
        except (TypeError, ValueError):
            implied = None
    log.info("FILLED: %.2f sh @ %.4f = $%.2f", filled, avg, cost)
    log.info("FEE CHECK: modeled $%.4f (rate %.3f exp %.1f, %.4f/sh)%s",
             modeled_fee, mk["fee_rate"], mk["fee_exponent"],
             modeled_fee / filled if filled else 0.0,
             f" | implied from fill legs $%+.4f" % implied if implied is not None else
             " | (response carried no maker/taker amounts to imply a fee)")
